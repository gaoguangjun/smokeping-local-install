...
Change_Source(){
	...
	cp /opt/smokeping-onekey/CentOS-Base.repo  /etc/yum.repos.d/
	cp /opt/smokeping-onekey/epel-7.repo /etc/yum.repos.d/
	cp /opt/smokeping-onekey/yum_local.repo /etc/yum.repos.d/
}
...
Download_Source(){
	...
	tar -xzvf /opt/smokeping-onekey/smokeping-2.6.11.tar.gz -C /root/
...
Configure_SomkePing(){
	...
	cp /opt/smokeping-onekey/config ./
	cp /opt/smokeping-onekey/Graphs.pm /opt/smokeping/lib/Smokeping/Graphs.pm
...
Configure_Nginx(){
	...
	cp /opt/smokeping-onekey/smokeping.conf /etc/nginx/conf.d/smokeping.conf
	rm -rf /etc/nginx/nginx.conf
	#wget -O /etc/nginx/nginx.conf https://raw.githubusercontent.com/ILLKX/smokeping-onekey/master/nginx.conf
	cp /opt/smokeping-onekey/nginx.conf /etc/nginx/nginx.conf 
}
...
Configure_Supervisor(){
	...
	cp /opt/smokeping-onekey/spawnfcgi.ini  /etc/supervisord.d/spawnfcgi.ini
...
Install_Tcpping(){
	...
	cp /opt/smokeping-onekey/tcpping ./
	chmod 777 tcpping
	mv tcpping /usr/bin/
